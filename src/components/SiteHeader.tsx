...
export function SiteHeader({ revealNav = true }: { revealNav?: boolean }) {
  return (
    <header
      className={`pointer-events-none fixed right-0 top-0 z-[60] isolate flex h-20 items-center pr-6 transition-opacity duration-500 ease-out supports-[mix-blend-mode:difference]:mix-blend-difference motion-reduce:transition-none ${
        revealNav ? "opacity-100" : "opacity-0"
      }`}
      aria-label="サイト内ナビゲーション"
    >
      <nav
        className={`flex w-max max-w-[min(100vw-1.5rem,100%)] flex-wrap items-end justify-end gap-4 rounded-lg p-6 text-[16px] leading-none text-[#333] supports-[mix-blend-mode:difference]:text-white ${
          revealNav ? "pointer-events-auto" : "pointer-events-none"
        }`}
      >
        <Link to="/#about" className="flex items-baseline">
          <span className="font-sans">ABOUT</span>
          <BrandSerif className="pl-1">
            <span className="text-[16px]">ME</span>
          </BrandSerif>
        </Link>
        <Link to="/#work" className="flex items-baseline">
          <span className="font-sans">RECENT</span>
          <BrandSerif className="pl-1">
            <span className="text-[16px]">WORKS</span>
          </BrandSerif>
        </Link>
        <Link to="/contact" className="flex items-baseline">
