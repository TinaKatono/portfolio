 ...
 * 弱く**あるべきなので、字を小さく・字間を広く・色を落として、値（本文色）に
 * 主役を譲る。以前は 3 箇所でサイズも字間も色もバラバラで、詳細ページに至っては
 * ラベルが中身より大きい逆転が起きていた。
 *
 * 新しくラベルを足すときもこれを使う。大文字にするのは呼び出し側の文字列で行う
 * （uppercase を当てると読み上げの都合が悪いため）。
 *
 * 色をこれ以上薄くしないこと。背景 #f5f7f8 に対して #546e7a で 5.0:1、
 * WCAG AA（小さい文字は 4.5:1）をぎりぎり満たす値。一段薄い #78909c だと
 * 3.2:1 で基準を割り、ラベルが並ぶ案件詳細のサイドバーは全体が沈んで見える。
 */
