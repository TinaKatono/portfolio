...
export type WorkDetail = {
  ...
  /**
   * 詳細右カラムのビジュアル（上から順。slot[i] が sections[i] の上、
   * 節数を超えたぶんは本文の下に続く）。
   *
   * 1 スロットに配列を入れると、その節の同じ位置に複数枚が続けて並ぶ。
   * 全体図とその拡大版のように、**離すと対応が読み取れない組**で使う。
   */
...
export const workDetails: Record<string, WorkDetail> = {
  ...
  work_3: {
    ...
    /*
      ...
      並びは本文に対応させてある。2 節目がシーケンス図の話なので、俯瞰（work_3_4）と
      その拡大（work_3_5）を 1 スロットにまとめ、同じ位置に続けて並べている。
      **この 2 枚を離さないこと。** 拡大版だと分かるのは隣り合っているからで、
      間に本文が入ると別の資料に見える。
      **スロットを 3 つ未満に減らさないこと。** 節数を下回るとグレーの空枠が出る。
    */
    galleryImages: [work3Gallery1, [work3Gallery4, work3Gallery5], work3Gallery3],
    sections: [
      {
